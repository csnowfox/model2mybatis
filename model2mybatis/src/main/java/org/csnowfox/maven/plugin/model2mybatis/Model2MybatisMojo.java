...
/**
 * 通过model文件生成mybatis generator格式的数据库访问类
 * maven插件入口
 * @goal echo
 * 
 */
public class Model2MybatisMojo extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		if(pathFile.isFile()){
			Model2MybatisJavaCode.createFiles(pathdao, pathsql, namesql,
			...
			try{
				...
				for(File tmpFile : subFile){
					...
					if(userMap.containsKey(tmpFileName)){
						...
						Model2MybatisJavaCode.createFiles(pathdao, pathsql, tmpFileName + "_" + namesql,
